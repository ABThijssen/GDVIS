...
#### Set up the environment and load + check data ------------------------------------------------------------------------------------
  ...
  if (triangle.input.list$plot_3D == TRUE) {
    ...
    required_pars <- c(
      "name_trait1",  "name_trait2", "name_trait3",  "name_trait1.con", "name_trait2.con", "name_trait3.con", "rg_trait1_trait2", "rg_se_trait1_trait2",  "rg_trait1_trait3",  "rg_se_trait1_trait3", "rg_trait2_trait3", 
      "rg_se_trait2_trait3" ,"h2_trait1","h2_se_trait1","h2_trait2","h2_se_trait2","h2_trait3","h2_se_trait3","pop.prev_trait1","pop.prev_trait2","pop.prev_trait3", "folder_location" )
...
#### Calculate filters -----------------------------------------------------------------------------------------------
  ...
  stop(filter1_error_message)
    ...
    if (h2_sub1.con - 1.96 * h2_se_sub1.con < 0) { 
     # filter2 <- 1
      filter2 <- 0 # need to remove this 
      # Need to uncomment everything
      # save_data <- FALSE
...
#### Calculate 3D parameters -----------------------------------------------------------------------------------------------
      ...
      # First check whether the triangle is possible
     # if (outcome_filter_1a > 1) { 
      #  (cli::cli_alert_danger("The rg between the subtypes is not significantly different from 1, 3D plot will not be informative and has the risk of being unreliable. It will not be plotted"))
        # cli::cli_abort(cli::col_red("The rg between the subtypes is not significantly different from 1, 3D plot will not be informative and has the risk of being unreliable. It will not be plotted"))
        # }
        
     
      # Some extra checks
   #   if(is.na(checkmode) == T) {
# 
#         a.rad_sub1.con_ext <- rg2angle(rg_sub1.con_ext)
#         a.rad_sub2.con_ext <- rg2angle(rg_sub2.con_ext)
#         cos_psi <- (cos(a.rad_sub2.con_ext) - cos(a.rad_sub1.con_ext) * cos(a.rad_sub1.con_sub2.con)) / (sin(a.rad_sub1.con_ext) * sin(a.rad_sub1.con_sub2.con))
# 
#         # Triangle fails if it is outside of [-1,1]
#         if( cos_psi < -1 | cos_psi > 1 )      {cli::cli_alert_danger("Cos psi invalid: triangle not possible")}
#         
#         internal_check_3D_a_sub2.con_ext_cos <- cos(a.rad_sub1.con_ext) * cos(a.rad_sub1.con_sub2.con) + sin(a.rad_sub1.con_ext) * sin(a.rad_sub1.con_sub2.con) * cos_psi
#         internal_check_3D_a_sub2.con_ext_cos_diff <- internal_check_3D_a_sub2.con_ext_cos - cos(a.rad_sub2.con_ext)
# 
#         # Triangle fails if the difference is too big
#         if( cos_psi > -1 & cos_psi < 1 ) {
#         if( abs(internal_check_3D_a_sub2.con_ext_cos_diff) > 0.02 )      {cli_alert_danger("Cosine rule failed: triangle not possible")} }
     # }
...
#### Save, print to console and return --------------------------------------------------------------------------------------------------------
    ...
    if(save_data == TRUE ) {
      ...
      if (plot_3D == TRUE & webversion == FALSE) {
        save_path <- paste0(folder_location, "/", filename, "_", name_ext, ".3D.triangle_parameters.RData")
      ...
      if (plot_3D == FALSE & webversion == FALSE) {
        save_path <- paste0(folder_location, "/", filename, ".2D.triangle_parameters.RData")
        save(triangle.output.list, file = save_path)
...
#### Calc 2D-2D --------------------------------------------------------------------------------------------------------------
    ...
    # Open connection
    if (webversion == FALSE) {
      con_log <- file(paste0(triangle1.folder_location,"/", triangle1.filename, ".with.",triangle2.filename,".2D.2D.log.txt"), open = "w")
      }
    
    if (webversion == TRUE) {
        con_path <- paste0(triangle1.folder_location,"/", triangle1.filename, ".with.",triangle2.filename,".2D.2D.log.txt")
        con_log <- file(con_path, open = "w")
      } 
    ...
    cli::cli_alert_success("GDVIS calc succesfully finished!")
    if(webversion == FALSE){
    return(paste0(triangle1.folder_location,"/",triangle1.filename, ".with.",triangle2.filename,".2D_2D.triangle_parameters.RData"))}
  ...
  #  return(mget(ls()))
    ...
    if (plot_CD == TRUE) {
      ...
      if( internal_check_rg_trait1.con_trait3.con - internal_check_rg_trait1_trait3 > 0.01) {stop("Error code: extension check d_trait3 not passed")}
      ...
      if( internal_check_rg_trait2.con_trait3.con - internal_check_rg_trait2_trait3 > 0.01) {stop("Error code: extension check d_trait3 not passed")}
      ...
      CD.triangle.output.list <- list(
        ...
        rg_se_trait1_trait2                                    = rg_se_trait1_trait2,
        rg_trait1_trait3                                       = rg_trait1_trait3,
        rg_se_trait1_trait3                                    = rg_se_trait1_trait3,
        rg_trait2_trait3                                       = rg_trait2_trait3,
        rg_se_trait2_trait3                                    = rg_se_trait2_trait3,
        h2_trait1                                              = h2_trait1,
        h2_se_trait1                                           = h2_se_trait1,
        h2_trait2                                              = h2_trait2,
        h2_se_trait2                                           = h2_se_trait2,
        h2_trait3                                              = h2_trait3,
        h2_se_trait3                                           = h2_se_trait3,
