...
#### plot 2D triangle -------------------------------------------------------------------------------- 
    ...
    plot <- ggplot() +
      ...
      # Plot lines 
      geom_line(aes(x = x, y = y, group = line_category, color = line_category, linewidth = line_category, linetype = line_category), data = subset(data, type == "line")) +
      scale_color_manual(values = c("sub2" = "#00B050", "sub1" = "#ED7D31", "sub1-sub2" = "#7030A0", "allcases" = "lightgray"),name = "Heritability",
                         # bquote(italic(h)^2 ~ obs[50/50])
                         labels = c("sub2" = h2_sub2.con, "sub1" = h2_sub1.con,"sub1-sub2" = h2_sub1.sub2, "allcases" = h2_allcases.con)) +
      
      scale_linewidth_manual(values = c("sub2" = sub2_line_width, "sub1" = sub1_line_width, "sub1-sub2" = subsub_line_width, "allcases" = allcases_line_width), guide = "none") +
      scale_linetype_manual(values = c("sub2" = sub2_line, "sub1" = sub1_line, "sub1-sub2" = subsub_line, "allcases" = "solid"), guide = "none") +
      
      # Plot the subgroup points
      geom_point(aes(x = x, y = y, fill = label_color), size = 2.5, shape = 21, color = "black", data = data_angle) + 
      scale_fill_manual(name = expression(italic(r)[g]~"(degrees)"), 
      ...
      geom_point(aes(x = x, y = y, shape = label_type, size = label_type), data = data_annot) + 
      scale_shape_manual(name = "", values = c("allcases" = 15, "population mean" = 8), 
                         labels = c("allcases" = name_allcases, "population mean" = "population mean")) +
      scale_size_manual(values = c("allcases" = 1.5, "population mean" = 1), guide = "none") +
      ...
      annotate("text", x = 0, y = y.con -0.04, label = plot_title, vjust = 2.5, hjust = 0.5, size = 5, fontface = "bold") + 
      
      # Fix legends
      theme(legend.position.inside = c(0.15, 0.5),                                    # Position the legend at the bottom left 
      ...
      guides(color = guide_legend(order = 1), fill = guide_legend(order = 2), shape = guide_legend(order = 3)) +
      ...
      if(rg_sub1.con_sub2.con < 1) {
        plot <- plot +
        geom_segment(aes(x = arrow.subsub_line.start[1], y = arrow.subsub_line.start[2], xend = arrow.subsub_line.end[1], yend = arrow.subsub_line.end[2]), color = "#7030A0", linewidth = 0.5, arrow = arrow(length = unit(0.2, "cm"), type = "closed")) +
        geom_segment(aes(x = arrow.allcases_line.start[1], y = arrow.allcases_line.start[2], xend = arrow.allcases_line.end[1], yend = arrow.allcases_line.end[2]), color = "lightgray", linewidth = 0.5, arrow = arrow(length = unit(0.2, "cm"), type = "closed")) +
        geom_segment(aes(x = arrow.sub1_line.start[1], y = arrow.sub1_line.start[2], xend = arrow.sub1_line.end[1], yend = arrow.sub1_line.end[2]), color = "#ED7D31", linewidth = 0.5, arrow = arrow(length = unit(0.2, "cm"), type = "closed")) +
        geom_segment(aes(x = arrow.sub2_line.start[1], y = arrow.sub2_line.start[2], xend = arrow.sub2_line.end[1], yend = arrow.sub2_line.end[2]), color = "#00B050", linewidth = 0.5, arrow = arrow(length = unit(0.2, "cm"), type = "closed"))
        }

      # Conditionally set x and y axis limits
      if (!is.null(x_lower) && !is.null(x_upper)) {
        plot <- plot + scale_x_continuous(limits = c(x_lower, x_upper))  }
      if (!is.null(y_lower) && !is.null(y_upper)) {
        plot <- plot + scale_y_continuous(limits = c(y_lower, y_upper))  }

      # Save plot
      ggsave(plot, file = paste0(folder_location,"/",filename, "_2D.triangle_plot.png"), height = 6, width = 12)
